...
#'   parameters. e.g. c(alpha = NA, beta = NA, mu = .6, theta = NA, phi = 1.6,
#'   sigma2 = NA) or just c(mu = .6, phi = 1.6)
...
.beta_parm <- function(b, to = "alpha.beta") {
  ...
  # mu.sigma2, or mu.phi input is named vector
  #
  ...
  # "role"))
  #
  # theta = alpha + beta which can also be expressed as ((mu (1 - mu)) / sigma2)
  # - 1
  #
  # and phi = 1 / (theta + 1)
  #
  # so alpha can be expressed as mu(((mu(1 - mu)) / sigma2) - 1) or as mu *
  # theta
  #
