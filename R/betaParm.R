...
#' @param B  \code{vector}  A named vector specifying non-NULL values for 2 parameters. e.g. c(alpha=NA, beta=NA, mu=.6, theta=NA, phi=1.6, sigma2=NA)
...
betaParm <- function(B, to='alpha.beta'){
  ...
  # sigma2 is the variance of the distribution;  sigma2 = (alpha*beta)/(((alpha + beta)^2)(alpha + beta + 1))
  # sigma2 can also be expressed as mu(1-mu)/(theta + 1)  #see below for a description of theta

  # sometimes the beta is parameterized in terms of the mean (mu) and the "sample size" (theta = alpha + beta (not a true sample size but minics that "role"))
