...
#' @param dat `vector` A vector of counts ordered in a manner consistent with
#'   output from the .cell_counts function.
#' @param SnR.vec `data.frame`  Each column corresponds to one reference test.
...
#'   population sampled.
#' @param N.vec `vector`  A named vector containing the sample size for each
#'   population sampled.
...
#'
#'
#' * \eqn{\delta}: `vector` The optimized values for the probability
#'  of a suspect result as a fraction of the non-correct test result for
#'  diseased samples.
#'
#' * \eqn{\gamma}: `vector` The optimized value for the probability
#' of a suspect result as a fraction of the non-correct test result for
