...
#' @param rep.iter logical (`TRUE`/`FALSE`) Indicates if updates should be
#'   printed regarding the number of iterations completed.  Defaluts to TRUE.
#' @param iter.n  integer indicating the frequency of updates for the number of
#'   iterations completed.  Defaluts to 50.
#' @param parm `vector`  Starting values for the optimization of the parameters
#'   of the experimental test.  If the experimental test has 2 states, this
#'   vector is of length two with elements corresponding to sensitivity and
...
#'   (\eqn{\pi}), the proportion of 1-Sn corresponding to the suspect region for
#'   disease positive samples (\eqn{\delta}), specificity (\eqn{\theta}), and
