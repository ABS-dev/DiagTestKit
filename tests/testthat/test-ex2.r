...
test_that("values", {
  ...
  expect_equal(ex2$calcVal$SpInterval,
               expected = round(c('5%' = 0.9260871, '100%' = 1), digits = 7),
               tolerance = 0.000001)

  #detailOut
  expect_equal(ex2$detailOut$Exp.Sn, ex2_detailOut$Exp.Sn, tolerance = 0.000001)
  expect_equal(ex2$detailOut$Exp.Sp, ex2_detailOut$Exp.Sp, tolerance = 0.000001)
  expect_equal(ex2$detailOut$Converge, ex2_detailOut$Converge, tolerance = 0)
  expect_equal(ex2$detailOut$Message, ex2_detailOut$Message)

  #input
  expect_equal(ex2$input$seed, expected = 4902342)
  # TODO: look at ex2$input$Sn.sims after naming fixed
  expect_equal(ex2$input$prev.sims, ex2_prevsims)
})
